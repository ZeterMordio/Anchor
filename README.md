...
Qwen3.5 reasoning ablations use `REASONING_MODE`: `option_a` (default) disables native chat-template thinking and keeps the explicit private `Thought:` field, while hard-canonicalizing public text to `Talk:` plus the first `Action:` line. `option_b` enables native Qwen thinking (`enable_thinking=True` in chat-template calls) and switches the role prompts to a native protocol: the model reasons inside private `<think>...</think>`, then outputs only public `Talk:` + `Action:`. Generated native-think blocks are kept in same-role private assistant history and stripped before opponent visibility/action parsing. The script logs native/explicit reasoning marker counts per iteration and can print raw/public buyer samples with `DEBUG_SAMPLE_BUYER_OUTPUTS`.
