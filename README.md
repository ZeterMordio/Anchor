...
Qwen3.5 reasoning ablations use `REASONING_MODE`: `option_a` (default) disables native chat-template thinking and keeps the explicit private `Thought:` field, while hard-canonicalizing public text to `Talk:` plus the first `Action:` line. `option_b` enables native Qwen thinking (`enable_thinking=True` in chat-template calls), keeps generated `<think>...</think>` blocks in same-role private assistant history, and still strips them before opponent visibility/action parsing. The script logs native/explicit reasoning marker counts per iteration.
